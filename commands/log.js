...
exports.run = async (client, message, args) => {
    ...
    var caseC = stringToBoolean(args[0]);

    if (typeof caseC === 'boolean') {
        if (caseC === settings.enabled) return message.channel.send(new MessageEmbed().setDescription(`Logging is already \`${caseC ? "enabled" : "disabled"}\``).setColor(0xFF0000));
        await client.db.set(message.guild.id, caseC, 'enabled');
        return message.channel.send(new MessageEmbed().setDescription(`Successfully \`${caseC ? "enabled" : "disabled"}\` logging`).setColor(0x00b9ff));
    }

    if (caseC === undefined && args[0].toLowerCase() === 'channel') {
...
function stringToBoolean(sting) {
	let value = 0;
	switch (sting.toLowerCase()) {
		case 'enable':
		case 'on':
			value = true;
			break;
